...
export function bfs(grid: Grid, start: Node) {
  let startNode = grid[start.row][start.col];
  const queue = [startNode];
  const visitedNodesInOrder = [];

  while (queue.length !== 0) {
    const vertex = queue.shift() as Node;
...
function updateNeighbors(node: Node, grid: Grid) {
...
export function shortestPathNodes(finishNode: Node) {
