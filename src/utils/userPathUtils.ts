import { Grid, Node } from "./utilities";

export function arrayDeepCopyOneDim(grid: Array<Node>) {
  ...
  let arrCopy: Array<Node> = JSON.parse(JSON.stringify(grid));
...
export function checkPathValidity(
  userPathArr: Node[],
  arrGrid: Grid,
  startNode: Node
): [boolean, string] {
...
function isSameNode(nodeA: Node, nodeB: Node) {
...
function isNoNeighboringUserNodes(neighbors: Node[], prevNode: Node) {
...
function isNonDeterministicUserPath(
  neighbors: Node[],
  prevNode: Node,
  currentNode: Node
) {
...
function removeNode(arr: Node[], node: Node) {
...
function getNextUserPathNode(neighbors: Node[], prevNode: Node) {
  for (const neighbor of neighbors) {
    if (neighbor.isUserNode && !isSameNode(neighbor, prevNode)) {
      return neighbor;
    }
...
function containsEndNode(neighbors: Node[]) {
